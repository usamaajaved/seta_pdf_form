 ...
 * Class Pushbutton
 *
 * This class allows a developer to create push buttons.
...
class Pushbutton extends SetaPDF_Core_Document_Page_Annotation_Widget
{
    ...
    protected function _getDaValues()
    {
        ...
        $parser->registerOperator(array('g', 'rg', 'k'), function($params) use (&$color) {
    ...
    protected function _createAppearance()
    {
        ...
        if (!$n) {
            $n = SetaPDF_Core_XObject_Form::create($document, array(0, 0, $width, $height));
        }
        ...
        if ($rotation != 0) {
            ...
            $n->getObject()->ensure()->getValue()->offsetSet('Matrix', new SetaPDF_Core_Type_Array(array(
        ...
        $appCharacteristics = $this->getAppearanceCharacteristics(true);
        $dict = $appCharacteristics->getDictionary();
        $textBlock->setText(SetaPDF_Core_Encoding::convertPdfString($dict->getValue('CA')->getValue(), 'UTF-16BE'), 'UTF-16BE');
